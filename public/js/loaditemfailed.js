(function(){
      ...
      socket.on('give-tile', function(tile){
        ...
        tileInHand.addEventListener('mousedown', function(event){
          ...
          if(hand.indexOf(event.target.parent) == -1){
            ...
            grid.removeChild(heldTile);
            stageOuter.addChild(heldTile);
            grid.updateCache();
  ...
  function drawBoardGrid(socket){
    ...
    grid.cache(0 - (BOARD_SIZE * BOARD_TILE_SIZE)/2, 0, BOARD_SIZE * BOARD_TILE_SIZE, BOARD_SIZE * BOARD_TILE_SIZE);
